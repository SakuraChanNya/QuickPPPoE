...
**“秒重连”指快速发现 RAS 掉线并发起重拨，不保证一秒内恢复互联网。** Windows PPPoE 发现、认证、旧会话释放和运营商限制均会影响时间。检测对象是 RAS 链路状态；若 RAS 仍显示已连接而上游网络不可达，本版本不会通过 ping 强制断线。电脑睡眠或程序退出期间不执行监测；唤醒后程序继续检查。
